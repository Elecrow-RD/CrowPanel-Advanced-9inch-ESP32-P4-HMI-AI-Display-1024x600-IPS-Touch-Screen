...
/* TX packet counter, incremented by the application each send. */
static uint32_t nrf24_tx_counter = 0;

/* Application-level callback invoked when an nRF24 packet is received. */
...
/**
 * @brief Transmit the current counter value as "NRF24_TX_Hello World:N".
 *
 * Uses the blocking transmit call; the counter is incremented by the
 * caller before each send so the text reflects the packet number.
 *
...
bool BSP_NRF2401::Send_pack_radio()
{
    ...
    snprintf(text, sizeof(text), "NRF24_TX_Hello World:%lu",
             (unsigned long)nrf24_tx_counter);
    size_t tx_len = strlen(text);
