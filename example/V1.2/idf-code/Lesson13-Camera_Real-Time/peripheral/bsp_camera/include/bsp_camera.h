...
/* SCCB (camera control) on I2C port 1: SCL=13, SDA=12. */
#define SCCB_MASTER_PORT 1
#define SCCB_GPIO_SCL    13
#define SCCB_GPIO_SDA    12

/* Maximum number of V4L2 capture buffers supported. */
...
/* Per-frame callback: receives the captured buffer and its geometry. */
...
/* Event bits used by the stream task event group. */
typedef enum {
...
/* Camera video runtime state. */
typedef struct {
    uint8_t *camera_buffer[MAX_BUFFER_COUNT];
    size_t camera_buf_size[MAX_BUFFER_COUNT];
    uint32_t camera_buffer_count;
