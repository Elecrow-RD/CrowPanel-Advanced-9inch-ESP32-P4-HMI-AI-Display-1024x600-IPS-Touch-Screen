 ...
 * @brief Teaching example that demonstrates how to receive SX1262 LoRa packets and display radio measurements.
 *
...
{
public:
  /** @brief Construct the lightweight SX1262 driver facade. */
  BSP_SX1262() {};

  /** @brief Destroy the lightweight SX1262 driver facade. */
  ~BSP_SX1262() {};
...
// Adapts the ESP32-P4 GPIO and SPI drivers to the RadioLib interface.
EspHal lora_hal;
Module *BSP_SX1262::bsp_sx_mod = nullptr;
SX1262 *BSP_SX1262::bsp_sx_radio = nullptr;

// Stores the completion result of the current asynchronous transmission.
static int lora_transmissionState = RADIOLIB_ERR_NONE;
// Interrupt callbacks set these flags; FreeRTOS tasks consume and clear them.
volatile bool lora_transmittedFlag = true;
volatile bool lora_receivedFlag = false;
// Records the packet length reported by the radio for the application callback.
...
{
public:
  /** @brief Construct the lightweight nRF24 driver facade. */
  BSP_NRF2401() {};

  /** @brief Destroy the lightweight nRF24 driver facade. */
  ~BSP_NRF2401() {};
 ...
 *--------------------------------------------------------------*/
/**
 * @brief Mark an SX1262 transmission as complete from the radio interrupt callback.
 *
 * Parameters: None.
 * @return None.
 * @note Called from the SX1262 packet-sent interrupt path.
 */
...
/**
 * @brief Initialize the SX1262 SPI interface and transmitter configuration.
 *
 * Parameters: None.
...
/**
 * @brief Stop SX1262 transmission activity and release its SPI bus.
 *
 * Parameters: None.
 * @return None.
...
/**
 * @brief Prepare and send the next lesson packet through the selected radio.
 *
 * Parameters: None.
...
/**
 * @brief Mark an SX1262 packet as ready from the radio interrupt callback.
 *
 * Parameters: None.
 * @return None.
 * @note Called from the SX1262 packet-received interrupt path.
 */
...
/**
 * @brief Initialize the SX1262 receiver and start interrupt-driven reception.
 *
 * Parameters: None.
 * @return ESP_OK on success; otherwise an ESP-IDF error code.
 * @note Called during lesson setup before the related hardware is used.
...
/**
 * @brief Stop SX1262 reception and release its SPI bus.
 *
 * Parameters: None.
 * @return None.
 * @note Called when the related hardware service is being stopped or reconfigured.
...
/**
 * @brief Read and report a packet after the radio signals that data is available.
 *
...
/**
 * @brief Initialize the nRF24 radio and configure its transmit pipe.
 *
 * Parameters: None.
 * @return ESP_OK on success; otherwise an ESP-IDF error code.
 * @note Called during lesson setup before the related hardware is used.
...
/**
 * @brief Stop nRF24 transmission activity and release its SPI bus.
 *
 * Parameters: None.
 * @return None.
 * @note Called when the related hardware service is being stopped or reconfigured.
...
/**
 * @brief Mark an nRF24 packet as ready from the radio interrupt callback.
 *
 * Parameters: None.
 * @return None.
 * @note Called from the nRF24 packet-received interrupt path.
 */
...
/**
 * @brief Initialize the nRF24 radio, receive pipe, and receive interrupt.
 *
 * Parameters: None.
 * @return ESP_OK on success; otherwise an ESP-IDF error code.
 * @note Called during lesson setup before the related hardware is used.
...
/**
 * @brief Stop nRF24 reception and release its SPI bus.
 *
 * Parameters: None.
 * @return None.
 * @note Called when the related hardware service is being stopped or reconfigured.
