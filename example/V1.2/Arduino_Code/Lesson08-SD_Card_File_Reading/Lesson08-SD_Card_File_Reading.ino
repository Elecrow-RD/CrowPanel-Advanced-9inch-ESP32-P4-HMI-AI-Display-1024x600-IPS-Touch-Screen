...
TaskHandle_t sd_task_handle = nullptr;   // Task handle for the SD card test task
...
esp_err_t write_string_file(const char *filename, const char *data)
{
    ...
    {
        SD_ERROR("Failed to write string data");
        return ESP_FAIL;
    }
...
esp_err_t read_string_file(const char *filename)
{
    ...
    {
        fclose(file);
        SD_ERROR("Failed to read string data");
        return ESP_FAIL;
    }
    fclose(file);
 ...
 * @note Called when the lesson requests the corresponding data operation.
 */
esp_err_t write_file(const char *filename, const void *data, size_t size)
{
 ...
 * @note Called when the lesson requests the corresponding data operation.
 */
esp_err_t write_file_seek(const char *filename, const void *data, size_t size, int32_t seek)
{
    size_t success_size = 0;
 ...
 * @note Called when the lesson needs to obtain or process new data.
 */
esp_err_t read_file(const char *filename, void *data, size_t size)
{
    size_t success_size = 0;
...
esp_err_t read_file_size(const char *read_filename)
{
    ...
    {
        fclose(read_file);
        SD_ERROR("Failed while reading file");
        return ESP_FAIL;
    }
    fclose(read_file);
    SD_INFO("File read success,success size =%zu", size);
    return ESP_OK;
}
 ...
 * @note Called when the lesson needs to obtain or process new data.
 */
esp_err_t read_write_file(const char *read_filename, const char *write_filename)
{
    size_t read_success_size = 0;
    ...
    if (ferror(read_file))
    {
        fclose(read_file);
        fclose(write_file);
        SD_ERROR("Failed while reading source file");
        return ESP_FAIL;
    }
    fclose(read_file);
    fclose(write_file);
    SD_INFO("File read and write success,success size =%zu", size);
    return ESP_OK;
}
...
void sd_task(void *param)   
{
    ...
    while (1)   // Infinite loop to perform read/write test
    {
        ...
        vTaskDelay(pdMS_TO_TICKS(200));   // Delay 200ms to allow SD card to complete internal operations
        ...
        vTaskDelay(pdMS_TO_TICKS(1000));   // Delay 1 second before repeating the test
...
void setup() {
    ...
    const BaseType_t task_result = xTaskCreatePinnedToCore(
        sd_task, "sd_task", 4096, NULL, 5, &sd_task_handle, 1
    );
    if (task_result != pdPASS) {
        MAIN_ERROR("Failed to create SD card task");
    }
}
