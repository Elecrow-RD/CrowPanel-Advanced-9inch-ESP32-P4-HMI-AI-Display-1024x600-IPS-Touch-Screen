 ...
 */
static size_t SendData(const char *data)
{
    const size_t len = strlen(data);     // Get the length of the input string
    const size_t txBytes = Serial1.write(data, len);  // Write string to UART1
 ...
 */
static size_t uart_read_response(char *buffer, size_t len, TickType_t timeout)
{
    size_t total = 0;  // Total number of bytes read
    size_t read_bytes = 0;  // Bytes read in current iteration
...
static bool send_at_command(const char *cmd, TickType_t timeout)
{
    ...
    if (SendData(cmd) != cmd_len || SendData("\r\n") != 2) {
        PRINTF_ERROR("UART write incomplete for command: %s", cmd);
        return false;
    }
    Serial1.flush();  // Wait until the complete command leaves the UART FIFO.
...
static bool connect_wifi()
{
    ...
    const int cmd_length = snprintf(cmd, sizeof(cmd), "AT+CWJAP=\"%s\",\"%s\"", WIFI_SSID, WIFI_PASS);
    if (cmd_length < 0 || static_cast<size_t>(cmd_length) >= sizeof(cmd)) {
        PRINTF_ERROR("WiFi credentials are too long for the AT command buffer");
        return false;
    }
...
void wifi_task(void *arg)
{
    ...
    if (!connected)
    {
        PRINTF_ERROR("[%s] Cannot connect to WiFi, stopping task", TAG);  // Log failure after all attempts
...
void setup() {
  ...
  // Create WiFi task with 4096 bytes stack, priority 5
    const BaseType_t task_result = xTaskCreate(wifi_task, "wifi_task", 4096, NULL, 5, NULL);
    if (task_result != pdPASS) {
        PRINTF_ERROR("Failed to create WiFi task");
    }
