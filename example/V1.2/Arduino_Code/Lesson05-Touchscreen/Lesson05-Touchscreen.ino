 ...
 * @brief Teaching example that demonstrates how to initialize the display and report touch coordinates.
 *
...
static esp_ldo_channel_handle_t ldo3_handle = nullptr;
static esp_ldo_channel_handle_t ldo4_handle = nullptr;
 ...
 *--------------------------------------------------------------*/
/**
 * @brief Initialize the hardware and services needed to initialize the display and report touch coordinates.
 *
 * Parameters: None.
 * @return None.
...
/**
 * @brief Continue the runtime workflow used to initialize the display and report touch coordinates.
 *
 * Parameters: None.
 * @return None.
...
void loop() {
  ...
  if (touch != nullptr) {
    ...
    // Core 3.3.8-compatible driver type; avoids the deprecated compatibility alias.
    ...
    if (!touch->isInterruptEnabled()) {
        ...
        delay(1);   // Yield when interrupt mode has no pending touch event.
    }
  } else {
