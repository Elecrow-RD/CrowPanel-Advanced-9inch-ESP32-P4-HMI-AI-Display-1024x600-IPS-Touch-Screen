...
/**
 * @brief Acknowledge completion of one automatic-white-balance statistic pass.
 *
 * @param awb_ctlr AWB controller that produced the event.
 * @param edata Statistics event supplied by the ISP driver.
 * @param user_data Optional callback context; unused by this board.
 * @return true to let the driver yield from the callback when required.
 * @note Invoked by the ISP after continuous AWB statistics are enabled.
 */
 ...
 *
 * @param handle Camera controller requesting a destination buffer.
 * @param trans Transaction populated with the buffer address and length.
 * @param user_data Template transaction registered during CSI setup.
 * @return false because this callback does not wake a higher-priority task.
 * @note Runs in interrupt context before the controller receives a frame.
 */
...
/**
 * @brief Acknowledge completion of a CSI frame transaction.
 *
 * @param handle Camera controller that completed the transfer.
 * @param trans Completed transaction descriptor.
 * @param user_data Registered callback context.
 * @return false because no task notification is required here.
 * @note Runs in interrupt context after a frame reaches PSRAM.
 */
...
/**
 * @brief Detect and configure the MIPI CSI image sensor over SCCB.
 *
 * The detector selects the 1024 x 600 RAW8 mode, applies mirror and exposure
 * settings, and starts the sensor stream consumed by the CSI controller.
 *
 * @param None.
 * @return ESP_OK on success or the first sensor/bus error.
 * @note Called once from camera_init() before CSI and ISP startup.
 */
...
/**
 * @brief Configure the two-lane CSI receiver and its frame callbacks.
 *
 * @param None.
 * @return ESP_OK when the controller is created and enabled.
 * @note Called once from camera_init() after sensor detection.
 */
...
/**
 * @brief Convert RAW8 camera data to display-ready RGB565 frames.
 *
 * The ISP also enables color adjustment, automatic white balance, automatic
 * exposure, and the color-correction matrix used by the preview image.
 *
 * @param None.
 * @return ESP_OK when every ISP stage is configured and enabled.
 * @note Called once from camera_init() after CSI controller setup.
 */
...
/**
 * @brief Allocate the frame buffer and start the complete camera pipeline.
 *
 * @param None.
 * @return ESP_OK when sensor, CSI, and ISP startup complete.
 * @note Called by the P4BspCamera constructor during board initialization.
 */
...
/**
 * @brief Wait until the CSI controller receives one fresh frame.
 *
 * @param None.
 * @return ESP_OK when my_trans contains the new RGB565 frame.
 * @note Called before preview display or remote image explanation.
 */
...
/**
 * @brief Mark the LVGL camera image as needing redraw.
 *
 * @param None.
 * @return None.
 * @note Called while the LVGL lock is held by a preview task.
 */
...
/**
 * @brief Create a full-screen LVGL image backed by the camera frame buffer.
 *
 * @param None.
 * @return None.
 * @note Called when continuous full-screen preview is explicitly requested.
 */
void camera_display()
{
